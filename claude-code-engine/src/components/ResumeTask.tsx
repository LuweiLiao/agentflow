...
export function ResumeTask({ onSelect, onCancel, isEmbedded = false }: Props): React.ReactNode {
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading AgentFlow-Code sessions…</Text>
        </Box>
        <Text dimColor>{retrying ? 'Retrying…' : 'Fetching your AgentFlow-Code sessions…'}</Text>
      </Box>
    );
  }
    ...
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading AgentFlow-Code sessions
        </Text>
  ...
  if (sessions.length === 0) {
    return (
      <Box flexDirection="column" padding={1}>
        <Text bold>
          No AgentFlow-Code sessions found
    ...
    case 'other':
      return (
        <Box marginY={1} flexDirection="row">
          <Text dimColor>Sorry, AgentFlow-Code encountered an error</Text>
        </Box>
      );
  }
}
