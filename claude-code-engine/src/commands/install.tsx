...
function Install({ onDone, force, target }: InstallProps): React.ReactNode {
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'AgentFlow-Code installation completed successfully', {
        display: 'system' as const,
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'AgentFlow-Code installation failed', {
        display: 'system' as const,
      });
    }
  }, [state, onDone]);
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      {state.type === 'installing' && (
        <Text color="claude">Installing AgentFlow-Code native build {state.version}...</Text>
      )}
        ...
        <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              AgentFlow-Code successfully installed!
            </Text>
          </Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install AgentFlow-Code native build',
