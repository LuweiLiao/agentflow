...
async function validatePluginManifest(filePath) {
  ...
  if (typeof parsed === "object" && parsed !== null) {
    ...
    if (strayKeys.length > 0) {
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          path: key,
          message: `Field '${key}' belongs in the marketplace entry (marketplace.json), ` + `not plugin.json. It's harmless here but unused \u2014 AgentFlow-Code ` + `ignores it at load time.`
        });
      }
      toValidate = stripped;
    }
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: "name",
        message: `Plugin name "${manifest.name}" is not kebab-case. AgentFlow-Code accepts ` + `it, but the Claude.ai marketplace sync requires kebab-case ` + `(lowercase letters, digits, and hyphens only, e.g., "my-plugin").`
      });
    }
    if (!manifest.version) {
      warnings.push({
...
//# debugId=040E2DA41DE5AB5E64756E2164756E21
//# sourceMappingURL=chunk-e1xn80a2.js.map
