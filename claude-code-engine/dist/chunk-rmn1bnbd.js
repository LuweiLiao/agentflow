...
class BridgeClient {
  ...
  async connect() {
    ...
    try {
      ...
      const durationMs = Date.now() - this.connectionStartTime;
      logger.error(`[${serverName}] Failed to create WebSocket after ${durationMs}ms:`, toLoggerDetail(error));
      trackEvent?.("chrome_bridge_connection_failed", {
        duration_ms: durationMs,
    ...
    this.ws.on("message", (data) => {
      try {
        ...
        logger.error(`[${serverName}] Failed to parse bridge message:`, toLoggerDetail(error));
      }
    });
  ...
  async handlePermissionRequest(message) {
    ...
    try {
      ...
      logger.error(`[${serverName}] Error handling permission request:`, toLoggerDetail(error));
...
${tabListText}`
  ...
  scheduleReconnect() {
    ...
    const delay = Math.min(2000 * 1.5 ** (this.reconnectAttempts - 1), 30000);
...
async function handleToolCallConnected(context, socketClient, name, args, permissionOverrides) {
  ...
  context.logger.warn(`[${context.serverName}] Unexpected result format from socket bridge: ${JSON.stringify(response)}`);
  return {
...
export { toLoggerDetail, localPlatformLabel, BridgeClient, createBridgeClient, BROWSER_TOOLS, createChromeSocketClient, createClaudeForChromeMcpServer, init_src };

//# debugId=85929E15FB93695664756E2164756E21
//# sourceMappingURL=chunk-rmn1bnbd.js.map
