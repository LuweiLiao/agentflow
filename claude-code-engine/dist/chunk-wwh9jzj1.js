...
async function performLogout({ clearOnboarding = false }) {
  const { flushTelemetry } = await import("./chunk-7mw97s81.js");
...
function clearChatGPTSettingsAuthMode() {
  delete process.env.OPENAI_AUTH_MODE;
  const userSettings = getSettingsForSource("userSettings") ?? {};
  const env = userSettings.env ?? {};
  const hasOpenAICompatibleConfig = Boolean(env.OPENAI_API_KEY ?? process.env.OPENAI_API_KEY) && Boolean(env.OPENAI_BASE_URL ?? process.env.OPENAI_BASE_URL);
...
async function call() {
  ...
  const message = /* @__PURE__ */ jsx_runtime.jsx(ThemedText, {
...
//# debugId=08EB3E08F6ECDBC164756E2164756E21
//# sourceMappingURL=chunk-wwh9jzj1.js.map
