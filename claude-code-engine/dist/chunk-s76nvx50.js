...
async function validateClickTarget(crop, lastScreenshot, xPercent, yPercent, takeFreshScreenshot, logger, gridSize = DEFAULT_GRID_SIZE) {
  ...
  try {
    ...
    logger.debug("[pixelCompare] validation error, skipping", toLoggerDetail(err));
    return { valid: true, skipped: true };
  }
}
var DEFAULT_GRID_SIZE = 9;
var init_pixelCompare = __esm(() => {
  init_types2();
});
...
async function runInputActionGates(adapter, overrides, subGates, actionKind) {
  ...
  if (frontmostTier !== undefined) {
    ...
    if (frontmostTier === "read") {
      ...
      return errorResult(`"${frontmost.displayName}" is granted at tier "read" \u2014 ` + `visible in screenshots only, no clicks or typing.` + (isBrowser ? " Use the Claude-in-Chrome MCP for browser interaction (tools " + "named `mcp__Claude_in_Chrome__*`; load via SearchExtraTools if " + "deferred)." : " No interaction is permitted; ask the user to take any " + "actions in this app themselves.") + TIER_ANTI_SUBVERSION, "tier_insufficient");
    }
...
async function handleToolCall(adapter, name, args, rawOverrides) {
  ...
  try {
    ...
    logger.error(`[${serverName}] tool=${name} threw: ${msg}`, toLoggerDetail(err));
...
//# debugId=5D497992AFC8419564756E2164756E21
//# sourceMappingURL=chunk-s76nvx50.js.map
