  ...
  switch (name) {
    ...
    case "Bash": {
      ...
      return {
        title: command,
        kind: "execute",
        content: _supportsTerminalOutput ? [{ type: "terminal", terminalId: toolUse.id }] : description ? [
          {
            type: "content",
...
// src/services/acp/permissions.ts
function createAcpCanUseTool(conn, sessionId, _getCurrentMode, clientCapabilities, cwd, onModeChange, isBypassModeAvailable) {
...
class AcpAgent {
  ...
  async replaySessionHistory(params) {
    try {
      const log = await getLastSessionLog(params.sessionId);
      if (!log || log.messages.length === 0)
        return;
      ...
      if (!session)
        return;
      await replayHistoryMessages(params.sessionId, messages, this.conn, session.toolUseCache, this.clientCapabilities, session.cwd);
    } catch (err) {
...
//# debugId=CA39AE739976A28164756E2164756E21
//# sourceMappingURL=chunk-95ag9dnt.js.map
