...
import type { WorkflowNode, WorkflowEdge, NodeStatus } from "./types.js";

// ── 应用状态 ──
const requirementInput = document.getElementById("requirement") as HTMLTextAreaElement;
...
let currentRunId = "";
let currentSseCancel: (() => void) | null = null;
let supSessionId = "";
...
function renderCanvas(): void {
  ...
  if (canvasEl) canvasEl.innerHTML = html;
...
async function startExecution(): Promise<void> {
  ...
  if (nodes.length === 0) { showToast("没有节点可执行"); return; }

  addLog(`🚀 提交执行: ${nodes.length} 节点`, "log-info");
  const execBtn = document.getElementById("execBtn") as HTMLButtonElement;
  ...
  try {
    ...
    // 订阅 SSE
    if (currentSseCancel) currentSseCancel();
    currentSseCancel = api.subscribeRunEvents(run_id, {
      ...
      onNodeComplete: (evt) => {
        graph.updateNodeStatus(evt.node_id, evt.status as NodeStatus);
        if (evt.result) graph.updateNode(evt.node_id, { result: evt.result.slice(0, 200) });
        renderCanvas();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (exampleSelect) {
    ...
    exampleSelect.addEventListener("change", () => {
      if (exampleSelect.value) requirementInput.value = exampleSelect.value;
    });
  }

  // 暴露到全局（用于 onClick）
  (window as any).__showNodeDetail = (id: string) => {
    const node = graph.getNode(id);
    if (node) showToast(`📋 ${node.label || id}\n${node.desc || "无描述"}\n状态: ${node.status || "pending"}`);
  };
});
